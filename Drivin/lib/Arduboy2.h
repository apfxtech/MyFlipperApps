...
public:
    ...
    void fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint8_t color) {
        ...
        for(int16_t yy = 0; yy < h; ++yy) {
            drawFastHLine(x, (int16_t)(y + yy), w, color);
        }
    }
    ...
    void fillCircle(int16_t x0, int16_t y0, int16_t r, uint8_t color) {
        ...
        for(int16_t yy = -r; yy <= r; ++yy) {
            for(int16_t xx = -r; xx <= r; ++xx) {
                const int32_t d = (int32_t)xx * (int32_t)xx + (int32_t)yy * (int32_t)yy;
                if(d <= rr) {
                    drawPixel((int16_t)(x0 + xx), (int16_t)(y0 + yy), color);
                }
            }
        }
    }
    ...
    void drawBitmap(
        ...
        for(int16_t yy = 0; yy < h; ++yy) {
            ...
            const uint8_t bit = (uint8_t)(1u << (yy & 7));
            for(int16_t xx = 0; xx < w; ++xx) {
                const uint8_t src = pgm_read_byte(bitmap + (page * w + xx));
                if(src & bit) {
                    drawPixel((int16_t)(x + xx), (int16_t)(y + yy), color);
                }
            }
        }
    }
