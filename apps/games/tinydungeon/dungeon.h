...
// object shading requires wall shading to work
  ...
  // draw dungeon floor (additional 192 bytes for graphics + some bytes for code)
...
public:
  ...
  uint8_t __attribute__ ((noinline)) getDownScaledBitmapData( const uint8_t x, uint8_t y, 
                                                              const uint8_t distance, const NON_WALL_OBJECT *object,
                                                              const bool useMask );
#else
  void    renderImage();
  uint8_t getDownScaledBitmapData( const uint8_t x, uint8_t y,
                                   const uint8_t distance, const NON_WALL_OBJECT* object,
                                   const bool useMask);
#endif
...
// lighting mask including fade in and out
