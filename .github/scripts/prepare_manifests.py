...
def patch_manifest_text(text: str):
    ...
    if cdef_idx is not None:
        ...
        match = re.match(r"^([ \t]*)cdefines\s*=\s*(.+?)(\s*,\s*)?$", code.rstrip("\r\n"))
        if not match:
        ...
        lines.insert(app_idx + 1, patched_line + newline)
...
def main():
    ...
    for src_dir in sorted(fam_dirs):
        ...
        with open(fam_path, "r", encoding="utf-8") as source:
            text = source.read()
        ...
        with open(staged_fam_path, "r", encoding="utf-8") as staged:
            staged_text = staged.read()

        patched_text, patched_line = patch_manifest_text(staged_text)

        with open(staged_fam_path, "w", encoding="utf-8") as staged:
            staged.write(patched_text)
    ...
    with open(os.environ["GITHUB_OUTPUT"], "a", encoding="utf-8") as output:
        output.write("app_ids<<EOF\n")
        for app_id, _target_name in app_entries:
            output.write(app_id + "\n")
        output.write("EOF\n")
