...
static void sound_system_init() {
    ...
    FuriMessageQueue* sound_queue = furi_message_queue_alloc(4, sizeof(SoundRequest));
    if(!sound_queue) return;

    FuriThread* sound_thread = furi_thread_alloc();
    if(!sound_thread) {
        furi_message_queue_free(sound_queue);
        return;
    }

    g_sound_queue = sound_queue;
    g_sound_thread = sound_thread;
...
void Platform::SetAudioEnabled(bool enabled) {
    if(!g_state) {
        if(!enabled) sound_system_deinit();
        return;
    }
...
uint8_t* Platform::GetScreenBuffer() {
    return g_state ? g_state->back_buffer : nullptr;
}
