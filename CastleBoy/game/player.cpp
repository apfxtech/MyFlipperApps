...
void Player::update() {
    ...
    // jump
    if(knockbackCounter == 0 && !ducking && attackCounter == 0 && grounded &&
    ...
    // vertical movement: walk
        ...
        if(offsetY > 0) {
            ...
            grounded = Entities::moveCollide(pos.x, pos.y, 0, 1, hitbox) ||
                       Map::collide(pos.x, pos.y + 1, hitbox);
        }
    ...
    if(velocityX != 0 &&
       (
           // normal speed
           ((knockbackCounter == 0) &&
            ab.everyXFrames(ducking ? PLAYER_SPEED_DUCK : PLAYER_SPEED_NORMAL)) ||
           // knockback speed
           ((knockbackCounter > 0) &&
            ab.everyXFrames(
                knockbackCounter < PLAYER_KNOCKBACK_FAST ? PLAYER_SPEED_KNOCKBACK_NORMAL :
                                                           PLAYER_SPEED_KNOCKBACK_FAST)))) {
        if(!Entities::moveCollide(pos.x, pos.y, velocityX, 0, hitbox) &&
           !Map::collide(pos.x + velocityX, pos.y, hitbox)) {
            pos.x += velocityX;
        }
    }
    ...
    if(attackCounter > 0) {
        if(--attackCounter <= PLAYER_ATTACK_CHARGE) {
            if(knifeAttack) {
                ...
                Entities::damage(
                    pos.x + (flipped ? -24 : 0), pos.y - (ducking ? 4 : 11), 24, 2, 2);
#ifdef DEBUG_HITBOX
                ab.fillRect(
                    pos.x + (flipped ? -24 : 0) - Game::cameraX,
                    pos.y - (ducking ? 4 : 12),
                    24,
                    2,
                    WHITE);
    ...
    if(knife) {
        ...
        if(Entities::damage(
               knifePosition.x - knifeHitbox.x,
               knifePosition.y - knifeHitbox.y,
               knifeHitbox.width,
               knifeHitbox.height,
               1)) {
            knife = false;
        ...
        if(knifePosition.x + knifeHitbox.width < Game::cameraX ||
           knifePosition.x > Game::cameraX + 128) {
            knife = false;
        }
    }
    ...
    Entity* entity =
        Entities::checkPlayer(pos.x - hitbox.x, pos.y - hitbox.y, hitbox.width, hitbox.height);
...
void Player::draw() {
    ...
    if(visible) {
        sprites.drawPlusMask(
            pos.x - SPRITE_ORIGIN_X - Game::cameraX,
            pos.y - SPRITE_ORIGIN_Y,
            player_plus_mask,
            frame + (flipped ? FRAME_FLIPPED_OFFSET : 0));

        if(attackCounter != 0 && !knifeAttack && attackCounter < PLAYER_ATTACK_CHARGE) {
            sprites.drawPlusMask(
                pos.x + (flipped ? -24 : 8) - Game::cameraX,
                pos.y - (ducking ? 4 : 12),
                flipped ? player_attack_left_plus_mask : player_attack_right_plus_mask,
                0);
        }
    }
    ...
    if(knife) {
        sprites.drawPlusMask(
            knifePosition.x - Game::cameraX, knifePosition.y, entity_knife_plus_mask, flipped);
    }

#ifdef DEBUG_HITBOX
    Box hitbox = ducking ? duckHitbox : normalHitbox;
    ab.fillRect(
        pos.x - hitbox.x - Game::cameraX, pos.y - hitbox.y, hitbox.width, hitbox.height, WHITE);
#endif
}
